layui
  ...
  .use(["form", "layer", "jquery", "layedit", "laydate", "upload"], function() {
    ...
    //分类下拉
    $.ajax({
      url: "/api/Category/getCategorylistalls",
      type: "GET",
      success: function(res) {
        if(res.code == 0 ){
          var options = '';
          for(var i = 0; i < res.result.length; i++){
            options+='<option value="'+res.result[i].id+'">'+res.result[i].category_text+'</option>';
          }
          $('.newsLook').html(options);
          form.render();
        }
      }
    });
    ...
    layui.upload({
      ...
      success: function(res) {
        ...
        if (res.code == 0) {
          articleCoverImg.src = CDNURL+res.data.src;
      ...
      $.ajax({
        ...
        success: function(res) {
          if (res.code == 0) {
            ...
            $("#articleCoverImg").attr("src", CDNURL+data.coverImg);
            $('.newsLook').find('option[value="'+data.typeName+'"]').attr("selected", true);
            if (data.state == 1) {
